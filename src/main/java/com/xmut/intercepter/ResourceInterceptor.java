...
public class ResourceInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        //对用户获取验证码，放行
        if (requestURI.indexOf("sendMs")>0){
            return true;
        }
        //对用户获取验证码，放行
        if (requestURI.indexOf("addUser")>0){
            return true;
        }
        if (requestURI.indexOf("addByDrugs")>0){
            return true;
        }
        if (requestURI.indexOf("borrowDrugs")>0){
            return true;
        }
